...
namespace Workflow
{
    public partial class ContractReviewCheckList_ME : Form
    {
        ...
        public ContractReviewCheckList_ME(string jobNo, string partNo, int workflow_ID, bool readOnly, bool hold, string startingStatus, bool canComplete)
        {
            InitializeComponent();
        ...
        {
            int startingRow = 4;
            int number_of_questions = 39;

            for (int i = startingRow; i < startingRow + number_of_questions*2; i += 2)
            {
            ...
            // if readonly disable buttons and ComboBox edits
            if (readOnly)
        ...
        {
            if (question1TextBox.Text == string.Empty)
            {
                button1.BackgroundImage = Image.FromFile("comment-icon.png");
            }
            else
                button1.BackgroundImage = Image.FromFile("comment-add-icon.png");
        }
        ...
        private void completeButton_Click(object sender, EventArgs e)
        {
            ...
            if (!canComplete)
            {
                MessageBox.Show("QA step has to be completed before ME checklist can be completed!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void ContractReviewCheckList_ME_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!hold)
            {
                ...
                using (OdbcConnection conn = new OdbcConnection(Globals.odbc_connection_string))
                {
                    ...
                    string query =
                        ...
                        "WHERE Job = '" + jobNoTextBox.Text.Trim() + "' AND Workflow_ID = '" + workflow_ID + "';";
