...
namespace Workflow
{
    public partial class QuickReleaseForm_Quality : Form
    {
        ...
        public QuickReleaseForm_Quality(string jobNo, string partNo, int workflow_ID, bool readOnly, bool hold, string startingStatus)
        {
            InitializeComponent();
            this.jobNo = jobNo;
            this.partNo = partNo;
        ...
        private void QuickReleaseForm_Load(object sender, EventArgs e)
        {
            ...
            using (OdbcConnection conn = new OdbcConnection(Globals.odbc_connection_string))
            {
                ...
                if (reader.Read())
                {
                    ...
                    statusLabel.Text = "Status: " + (reader.IsDBNull(10) ? string.Empty : reader.GetString(10));

                    // check if StartedOn is NULL, this will tell you if it's the first time
                    firstOpen = reader.IsDBNull(13);
                }
            }
        ...
        private void QuickReleaseForm_Quality_FormClosing(object sender, FormClosingEventArgs e)
        {
